// src/pages/admin/PatientManagement.jsx (updated)
...
const PatientManagement = () => {
  ...
  const fetchPatients = async () => {
    try {
      ...
      toast.error("Failed to fetch patients. Please log in."); // MODIFIED
    }
  };
  ...
  const handleAddPatient = async (e) => {
    ...
    if (!newPatient.name || !newPatient.nik) {
      toast.error("Please fill in at least the NIK and Name fields."); // MODIFIED
      return;
    }
    try {
      await axios.post(API_URL, newPatient, getAuthHeaders());
      toast.success("Patient successfully added!"); // MODIFIED
      setShowAddForm(false);
      setNewPatient({ nik: "", name: "", gender: "", birthdate: "", bloodType: "", contact: "", address: "", medicalHistory: "" });
      fetchPatients();
    } catch (error) {
      console.error("Error adding patient:", error);
      toast.error(error.response?.data?.message || "Could not add patient."); // MODIFIED
    }
  };
  ...
  const handleUpdatePatient = async (e) => {
    ...
    try {
      ...
      toast.success("Patient details successfully updated!"); // MODIFIED
      setIsEditModalOpen(false);
      fetchPatients();
    } catch (error) {
      console.error("Error updating patient:", error);
      toast.error(error.response?.data?.message || "Could not update patient."); // MODIFIED
    }
  };
  ...
  // MODIFIED: This function confirms and executes the deletion
  const handleConfirmDelete = async () => {
    if (!patientToDeleteId) return;

    try {
      await axios.delete(`${API_URL}/${patientToDeleteId}`, getAuthHeaders());
      toast.success("Patient successfully deleted!"); // MODIFIED
      ...
      toast.error(error.response?.data?.message || "Could not delete patient."); // MODIFIED
      setIsDeleteModalOpen(false); // Close the modal on error too
    }
  };
  ...
  return (
    <>
            ...
            {editingPatientData && (
              <form onSubmit={handleUpdatePatient}>
                ...
                <div className="flex justify-between items-center mt-6">
                  <button
                    type="button"
                    // MODIFIED: This button now opens the delete modal
                    onClick={() => handleOpenDeleteModal(editingPatientData._id)}
      ...
      {/* NEW: Render the confirmation modal for deleting a patient */}
      <ConfirmationModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={handleConfirmDelete} title="Delete Patient" confirmText="Delete Patient">
        <p>
          Are you sure you want to delete this patient record? <br /> This action cannot be undone.
        </p>
      </ConfirmationModal>
    </>
  );

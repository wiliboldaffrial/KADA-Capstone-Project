...
const PATIENT_API_URL = 'http://localhost:5000/api/patients'; // NEW: Patient API URL
...
const AppointmentSchedule = () => {
  ...
  const [patientList, setPatientList] = useState([]); // NEW: State for the list of patients
  ...
  // NEW: Function to fetch the list of patients
  ...
  useEffect(() => {
    fetchAppointments();
    fetchPatientList(); // Fetch both appointments and patients on load
  }, []);
  ...
  const handleEditClick = () => {
    const formattedDateTime = selectedAppointment.dateTime 
        ? new Date(new Date(selectedAppointment.dateTime).getTime() - (new Date().getTimezoneOffset() * 60000)).toISOString().slice(0, 16)
        : '';
        ...
        <div className={`flex-1 transition-all duration-300 p-6 ${isSidebarCollapsed ? "ml-16" : "ml-64"}`}>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md mb-8">
              ...
              <form onSubmit={handleAddAppointment}>
                <div className="grid grid-cols-2 gap-4">
                  {/* MODIFIED: Replaced text input with SearchablePatientInput */}
                  ...
                  <div className="flex flex-col">
                    ...
                    <input type="datetime-local" id="dateTime" name="dateTime" value={newAppointment.dateTime} onChange={(e) => setNewAppointment(prev => ({...prev, dateTime: e.target.value}))} className="w-full px-3 py-2 border border-gray-300 rounded-md" required />
                  </div>
                  <div className="flex flex-col">
                    <label htmlFor="notes" className="mb-1 text-sm font-medium text-gray-600">Notes</label>
                    <textarea id="notes" name="notes" value={newAppointment.notes} onChange={(e) => setNewAppointment(prev => ({...prev, notes: e.target.value}))} rows="1" className="w-full px-3 py-2 border border-gray-300 rounded-md"></textarea>
                  </div>
                </div>
                <div className="flex justify-end mt-6">
                  <button type="submit" className="bg-blue-600 text-white font-semibold px-6 py-2 rounded-lg hover:bg-blue-700 transition">Add Appointment</button>
                </div>
              </form>
            </div>
          )}
          ...
          <div className="bg-white rounded-lg drop-shadow-xl p-4 mt-4">
            ...
            <div className="flex items-center justify-between mb-4 bg-blue-600">
                <button onClick={goToPreviousMonth} className="p-2 rounded-full hover:bg-gray-100"><ChevronLeft className="w-6 h-6 text-blue-200" /></button>
                <h2 className="text-xl font-semibold text-white px-20 py-4 rounded-lg">{format(currentDate, "MMMM yyyy")}</h2>
                <button onClick={goToNextMonth} className="p-2 rounded-full hover:bg-gray-100"><ChevronRight className="w-6 h-6 text-blue-200" /></button>
            </div>
            <div className="grid grid-cols-7 text-center font-semibold text-gray-600 border-b mb-2 pb-2">{daysOfWeek.map((day) => <div key={day}>{day}</div>)}</div>
            <div className="grid grid-cols-7">
                {Array.from({ length: startingDayIndex }).map((_, index) => <div key={`empty-${index}`} className="border-r border-b"></div>)}
                {daysInMonth.map((day, index) => {
                    ...
                    return (
                        <div key={index} className="border-r border-b p-2 h-36 relative">
                            <div className="flex justify-center"><div className={`w-8 h-8 flex items-center justify-center rounded-full font-medium ${isToday(day) ? "bg-blue-600 text-white" : "text-gray-800"}`}>{format(day, "d")}</div></div>
                            <div className="mt-1 space-y-1 overflow-y-auto max-h-24">
                                {dayAppointments.map((app) => (
                                    <div key={app._id} onClick={() => setSelectedAppointment(app)} className="bg-blue-500 text-white text-xs rounded-md p-1 truncate cursor-pointer hover:bg-blue-700">{app.patient} - {app.time}</div>
                                ))}
                            </div>
                        </div>
                    );
          ...
          {selectedAppointment && (
            <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50">
              <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-lg relative animate-fade-in">
                <button onClick={() => { setSelectedAppointment(null); setIsEditing(false); }} className="absolute top-4 right-4 text-gray-400 hover:text-gray-800 transition"><X size={24} /></button>
                {isEditing ? (
                  <form onSubmit={handleUpdateAppointment}>
                    ...
                    <div className="flex justify-between items-center">
                      <button type="button" onClick={() => handleOpenDeleteModal(selectedAppointment._id)} className="flex items-center gap-2 bg-red-100 text-red-600 font-semibold px-4 py-2 rounded-lg hover:bg-red-200 transition"><Trash2 size={16} /> Delete</button>
                      <div className="flex gap-4">
                        <button type="button" onClick={() => setIsEditing(false)} className="bg-gray-200 text-gray-800 font-semibold px-4 py-2 rounded-lg hover:bg-gray-300 transition">Cancel</button>
                  ...
                  <div>
                    <div className="flex justify-between items-start"><h3 className="text-2xl font-bold text-gray-800 mb-6 border-b pb-4 flex-grow">Appointment Details</h3></div>
                    <div className="space-y-4 text-gray-700">
                      ...
                      <div className="flex justify-end pt-4">
                        <button onClick={handleEditClick} className="flex items-center gap-2 bg-blue-600 text-white font-semibold px-4 py-2 rounded-lg hover:bg-blue-700 transition"><Edit size={16} /> Edit Appointment</button>
                      </div>
                    </div>
                  </div>
                )}

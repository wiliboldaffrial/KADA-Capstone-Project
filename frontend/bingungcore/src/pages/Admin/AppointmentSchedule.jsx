...
const AppointmentSchedule = () => {
  ...
  const [appointments, setAppointments] = useState([]); // MODIFIED: Start with an empty array
  ...
  // MODIFIED: Standardized 'patientName' to 'patient' to match the backend model
  ...
  const fetchAppointments = async () => {
    try {
      ...
      // Format data for display after fetching
      ...
      toast.error('Failed to fetch appointments. Please make sure you are logged in.');
    }
  };

  // NEW: Fetch data when component mounts
  useEffect(() => {
    fetchAppointments();
  }, []);
  ...
  const handleAddAppointment = async (e) => {
    ...
    try {
      await axios.post(API_URL, newAppointment, getAuthHeaders());
  ...
  const handleUpdateAppointment = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`${API_URL}/${editingData._id}`, editingData, getAuthHeaders());
  ...
  const handleConfirmDelete = async () => {
    if (appointmentToDelete) {
      try {
        await axios.delete(`${API_URL}/${appointmentToDelete}`, getAuthHeaders());
              ...
              <form onSubmit={handleAddAppointment}>
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col">
                    ...
                    {/* MODIFIED: 'name' attribute is now 'patient' */}
                    <input type="text" id="patientName" name="patient" value={newAppointment.patient} onChange={handleInputChange} className="w-full px-3 py-2 border border-gray-300 rounded-md" required />
                  </div>
                  <div className="flex flex-col">
                  ...
                  <div key={index} className="border-r border-b p-2 h-36 relative">
                    ...
                    <div className="mt-1 space-y-1 overflow-y-auto max-h-24">
                      {dayAppointments.map((app) => (
                        // MODIFIED: Use _id from MongoDB for the key
                ...
                {isEditing ? (
                  <form onSubmit={handleUpdateAppointment}>
                    ...
                    <div className="flex justify-between items-center">
                      <button type="button" onClick={() => handleOpenDeleteModal(selectedAppointment._id)} className="flex items-center gap-2 bg-red-600 text-white font-semibold px-4 py-2 rounded-lg hover:bg-red-700 transition"><Trash2 size={16} /> Delete</button>

// src/pages/admin/AppointmentSchedule.jsx (updated)
...
const AppointmentSchedule = () => {
  ...
  // This function now confirms the deletion after the modal is shown
  const handleConfirmDelete = () => {
    if (appointmentToDelete) {
      setAppointments(appointments.filter((app) => app.id !== appointmentToDelete));
      toast.success("Appointment successfully deleted!");
      setSelectedAppointment(null); // Close the edit/details modal
      setIsEditing(false);
      setDeleteModalOpen(false); // Close the confirmation modal
      setAppointmentToDelete(null); // Reset the state
    }
  };
  ...
  return (
    <>
                  ...
                  <div key={index} className="border-r border-b p-2 h-36 relative">
                    ...
                    <div className="mt-1 space-y-1 overflow-y-auto max-h-24">
                      {dayAppointments.map((app) => (
                        <div key={app.id} onClick={() => setSelectedAppointment(app)} className="bg-blue-500 text-white text-xs rounded-md p-1 truncate cursor-pointer hover:bg-blue-700">
                ...
                {isEditing ? (
                  ...
                  <form onSubmit={handleUpdateAppointment}>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <button type="button" onClick={() => handleOpenDeleteModal(selectedAppointment.id)} className="flex items-center gap-2 bg-red-100 text-red-600 font-semibold px-4 py-2 rounded-lg hover:bg-red-200 transition">
      ...
      <ConfirmationModal isOpen={isDeleteModalOpen} onClose={() => setDeleteModalOpen(false)} onConfirm={handleConfirmDelete} title="Delete Appointment" confirmText="Delete Appointment">
        <p>
